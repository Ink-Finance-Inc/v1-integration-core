...
import * as EscrowManagerABI from "../../artifacts/contracts/products/EscrowManager.sol/EscrowManager.json";
import * as InkEnvelopeFactoryABI from "../../artifacts/contracts/products/InkEnvelopeFactory.sol/InkEnvelopeFactory.json";
...
export async function EscrowManagerFixture(_wallets: Wallet[], _mockProvider: MockProvider) {

    const signers = await ethers.getSigners();
    const escrowManager = await deployContract(signers[0], EscrowManagerABI, []);
    await escrowManager.deployed();

    return {escrowManager};
}

export async function InkEnvelopeFactoryFixture(_wallets: Wallet[], _mockProvider: MockProvider) {

    const signers = await ethers.getSigners();
    const envelopeFactory = await deployContract(signers[0], InkEnvelopeFactoryABI, []);
    await envelopeFactory.deployed();
